...
namespace POSales
{
    public partial class Supplier : Form
    {
        ...
        public void LoadSupplier()
        {
            ...
            dr.Close();
            cn.Close();
            dgvSupplier.ReadOnly = true; // Establecer el DataGridView como solo lectura
        }
        ...
        private void dgvSupplier_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("¿Eliminar este registro? Haga clic en Sí para confirmar", "CONFIRM", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    cn.Open();
                    cm = new SqlCommand("DELETE FROM tbSupplier WHERE id LIKE '" + dgvSupplier.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", cn);
                    cm.ExecuteNonQuery();
                    cn.Close();
                    MessageBox.Show("El registro se ha eliminado correctamente.", "Delete Record", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            LoadSupplier();
        }
