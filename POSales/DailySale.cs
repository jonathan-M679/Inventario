...
namespace POSales
{
    public partial class DailySale : Form
    {
        ...
        public DailySale(MainForm mn)
        {
            ...
            main = mn;
            solduser = main.lblUsername.Text; // Asigna el nombre del usuario
            LoadCashier();
        }

        public DailySale(MainFormRoot mn)
        {
            InitializeComponent();
            cn = new SqlConnection(dbcon.myConnection());
            main2 = mn;
            solduser = main2.lblUsername.Text; // Asigna el nombre del usuario
            LoadCashier();
        }
        ...
        public void LoadCashier()
        {
            ...
            cn.Open();
            cm = new SqlCommand("SELECT * FROM tbUser WHERE role LIKE 'Cajero'", cn);
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        public void LoadSold()
        {
            ...
            foreach (DataGridViewColumn column in dgvSold.Columns)
            {
    ...
    private void dgvSold_CellContentClick_1(object sender, DataGridViewCellEventArgs e, string username)
        {
            string colName = dgvSold.Columns[e.ColumnIndex].Name;
            if (colName == "Cancel")
            {
        ...
        private void dgvSold_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
            if (main != null)
            {
                dgvSold_CellContentClick_1(sender, e, main.lblUsername.Text);
            }
            else if (main2 != null)
            {
                dgvSold_CellContentClick_1(sender, e, solduser);
            }
        }
