...
namespace POSales
{
    public partial class Cashier : Form
    {
        ...
        {
            slide(btnClear);
            if (MessageBox.Show("¿Eliminar todos los artículos del carrito?", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                cn.Open();
                cm = new SqlCommand("Delete from tbCart where transno like'" + lblTranNo.Text + "'", cn);
                cm.ExecuteNonQuery();
                cn.Close();
                MessageBox.Show("Todos los elementos se han eliminado con éxito", "Remove item", MessageBoxButtons.OK, MessageBoxIcon.Information);
                LoadCart();
            }
        }
        ...
        private void btnLogout_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("No se puede cerrar la sesión. Por favor, cancele la transacción.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        ...
        public void LoadCart()
        {
            try
            {
                ...
                foreach (DataGridViewColumn column in dgvCash.Columns)
                {
        ...
        private void txtBarcode_TextChanged(object sender, EventArgs e)
{
    try
    {
        ...
        else
        {
            ...
            if(dr.HasRows)
            {
                ...
                // Clear the barcode text after successfully adding it to the cart
    ...
    catch (Exception ex)
    {
        ...
        public void AddToCart(string _pcode, double _price,int _qty)
        {
            try
            {
                ...
                if (found)
                {
                    ...
                    {
                        MessageBox.Show("No se puede proceder. La cantidad restante disponible es " + qty, "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
                    cn.Open();
        ...
        private void dgvCash_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Delete")
            {
                if (MessageBox.Show("Eliminar este artículo", "Remove item", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    dbcon.ExecuteQuery("Delete from tbCart where id like'" + dgvCash.Rows[e.RowIndex].Cells[1].Value.ToString() + "'");
                    MessageBox.Show("Los elementos se han eliminado con éxito", "Remove item", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    LoadCart();
                }
            }
            else if (colName == "colAdd")
            {
                ...
                {
                    MessageBox.Show("La cantidad restante disponible es " + i + "!", "Out of Stock", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
            else if (colName == "colReduce")
            {
                ...
                if (currentQty > 1)
                {
                    dbcon.ExecuteQuery("UPDATE tbCart SET qty = qty - 1 WHERE transno LIKE '" + lblTranNo.Text + "'  AND pcode LIKE '" + dgvCash.Rows[e.RowIndex].Cells[2].Value.ToString() + "'");
                    LoadCart();
                }
                else
                {
                    MessageBox.Show("La cantidad no puede ser menor que 1!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        }
